...
**I was the only engineering manager**, so I defined and executed on all relevant concerns (hiring, performance reviews, career progression, 1:1s, training,  process definition, role/team structuring, etc) and provided support on most admin duties (finances, legal, processes, office, etc).
...
Worked on re-hauling the business owner activity feed microservice, migrating from Gearman to Kafka, and modernizing its clustering infra. **Helped trailblaze the introduction of Kafka company-wide**. Shaped and implemented tools and packages that became standard practice.
